...
export default {
  methods: {
    calculateDistance(elem, mouseX, mouseY) {
      return Math.floor(
        Math.sqrt(
          Math.pow(
            mouseX - (elem.getBoundingClientRect().left + elem.offsetWidth / 2),
            2
          ) +
            Math.pow(
              mouseY -
                (elem.getBoundingClientRect().top + elem.offsetHeight / 2),
              2
            )
        )
      );
  ...
  mounted() {
    this.$nextTick(function() {
      var example = this.$refs.hero;
      let vm = this;
      example.onmousemove = function(e) {
        let mX = e.pageX;
        let mY = e.pageY;
        let distance = vm.calculateDistance(vm.$refs.globe, mX, mY);
        vm.$refs.blur.style.opacity = distance / 400;
        ...
        var breathAnimation = anime({
          ...
          update: function(ins) {
            aimations.forEach(function(animation, i) {
              var percent =
                (1 - Math.sin(i * 0.35 + 0.0008 * ins.currentTime)) / 2;
